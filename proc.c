...
/*When a process is first a cmostime().llocated, set the begin_date to the “current”
...
startup. Finding the right call to get the date/time is a little awkward. I recommend you look in
...
tad awkward, because you must pass an address or pointer. Remember that putting an & in
front of a variable/structure when calling a function will pass the address of the
variable/structure. Decide to ignore the goto and the label found in the allocproc()
function; we won’t talk about them.
...
of code into the scheduler() routine. 

One is just before the newly chosen process is 
scheduled and 

the other is just after that process returns back to the scheduler. 

The first place (before the newly chosen process runs), is pretty easy to find. Look for the place where the
state of the process is set to RUNNING. I dropped an #ifdef block just after that. That block
...
the kernel function uptime() is defined and implemented (as a user code callable system
function sys_uptime(), in the file sysproc.c), you cannot directly call it from within
kernel. You have a choice to either 1) replicate the capability of uptime() in the
scheduler() code (which I do not like), or 2) make a new function that returns the same thing (which I do like). If you decide to do the second option, 

I recommend you have sys_uptime() directly call your new function (which I call suptime()). I went with option
2; I don’t like to duplicate code.

Now, about that second place to drop in a few lines of code. What do you think the following 2
lines of code from the scheduler() function do?1
